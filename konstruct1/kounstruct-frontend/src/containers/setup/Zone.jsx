...
// Color tokens - must match Level.jsx
const ORANGE = "#b54b13";
...
function Zone({ nextStep }) {
  ...
  const accent = isDark ? DARK_ORANGE : ORANGE;
  const accentDark = isDark ? DARK_ORANGE_DARK : ORANGE_DARK;
  const inputBg = isDark ? DARK_BG : "#fff";
  const disabledColor = isDark ? "#9e9e9e" : "#e5b48c";
  const errorColor = isDark ? "#e65100" : "#e65100";
  const zoneBg = isDark ? "#322509" : "#fffbe6";
  const subZoneBg = isDark ? "#292004" : "#fffde7";
  const yellow = isDark ? DARK_ORANGE : "#ffe066";
  ...
  if (!zoneData || Object.keys(zoneData).length === 0) {
    return (
      <div className="flex items-center justify-center min-h-screen" style={{ background }}>
        <div className="text-center p-8 rounded-lg" style={{ background: cardBg, color: textColor }}>
          <AlertCircle className="w-12 h-12 mx-auto mb-4" style={{ color: accent }} />
  ...
  return (
    <div className="min-h-screen p-4" style={{ background }}>
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-4 p-4 rounded-lg" style={{ background: cardBg }}>
          <h2 className="text-xl font-bold" style={{ color: accentDark }}>
            Zone Configuration
          </h2>
        ...
        {/* Batch Add */}
        <div className="mb-4 p-3 rounded-lg flex items-center gap-3" style={{ background: cardBg }}>
          <span className="text-sm font-medium" style={{ color: textColor }}>Quick Add:</span>
          <input
            type="number"
            min="1"
        ...
        <div className="space-y-3">
          {Object.keys(zoneData).map((towerId) => (
            <div key={towerId} className="border rounded-lg" style={{ backgroundColor: cardBg, borderColor }}>
              {/* Tower Header */}
              <button
                onClick={() => setExpandedTowers(prev => ({ ...prev, [towerId]: !prev[towerId] }))}
                className="w-full flex items-center justify-between p-3"
                style={{ color: accentDark, background: "none" }}
              >
              ...
              {expandedTowers[towerId] && (
                <div className="border-t px-3 pb-3" style={{ borderColor }}>
                  {sortedFloors(zoneData[towerId]?.floors).map((floor) => {
                    ...
                    return (
                      <div key={floor.level_name} className="mt-3 border rounded" style={{ borderColor }}>
                        {/* Floor Header */}
                        <button
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-xs" style={{ color: disabledColor }}>
                              ...
                              {floor.zones && floor.zones.length > 0 && (
                                <div className="flex items-center gap-2 text-sm">
                                  <button
                                    ...
                                    className={`px-2 py-1 border rounded ${allSelected ? 'bg-yellow-400 text-white' : ''}`}
                                  ...
                                  <button
                                    ...
                                    style={{
                                      ...
                                      cursor: !hasSelectedZones ? "not-allowed" : "pointer"
                                    }}
                                  >
                                    Delete Selected
                                  </button>
